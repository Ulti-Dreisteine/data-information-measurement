...
class Cell(object):
    ...
    def __init__(self, arr: np.ndarray) -> None:
        ...
        self.bounds = self._get_arr_bounds()
        self.area = self._cal_area()

    def _get_arr_bounds(self):
        return list(zip(np.min(self.arr, axis=0), np.max(self.arr, axis=0)))
    ...
    def equiprob_partition(self) -> tuple:
        ...
        return Cell(part_arr_ul), Cell(part_arr_ur), Cell(part_arr_ll), Cell(part_arr_lr)
...
def recursively_partition(cell: Cell, eps: float=1E-2, N_min: int = 30) -> Tuple[Cell, Cell, Cell, Cell]:
    """递归分裂离散化
...
if __name__ == '__main__':
    ...
    partition_ul, partition_ur, partition_ll, partition_lr = recursively_partition(cell)
