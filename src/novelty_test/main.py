...
def rmic(x: np.ndarray, y: np.ndarray, x_type: str) -> float:
    mutual_info_coeff = RefinedMutualInfoCoeff(x, y, x_type)
    entropy = mutual_info_coeff.cal_mic()
    return entropy
...
if __name__ == '__main__':
    ...
    N = 1000
    novelty_test = None
    method = 'rmic'  # 可选: 'mic', 'rmic', 'kl_knn'
    ...
    for func in FUNCS2TEST:
        ...
        for radius in np.arange(0.0, 2.0 + 0.025, 0.025):
            ...
            if method == 'rmic':
                x_type = 'categoric' if func == 'categorical' else 'numeric'
                entropy = eval(method)(x_noise, y_noise, x_type)
            else:
                entropy = eval(method)(x_noise, y_noise)
            
            # 选择以R2为横坐标还是以noise级别为横坐标.
        ...
        entropy_results = pd.DataFrame(
            entropy_results, columns=['time', '{}'.format(func)]
        )
        dts = DataTimeStampSerialization(
            entropy_results, start_stp=0.0, end_stp=2.0, stp_step=0.025)
        entropy_results, _ = dts.serialize_time_stamps()

        if novelty_test is None:
            novelty_test = entropy_results
        else:
            novelty_test = pd.concat(
                [novelty_test, entropy_results.loc[:, ['{}'.format(func)]]], axis=1)
